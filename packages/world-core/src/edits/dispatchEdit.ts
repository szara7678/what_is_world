import type { EditCommand, Structure, WorldState } from "@wiw/shared";
...
export const dispatchEdit = (world: WorldState, cmd: EditCommand): EditResult => {
  switch (cmd.type) {
    case "PLACE_TILE": {
      if (!inBounds(world, cmd.x, cmd.y)) return { ok: false, message: "out_of_bounds" };
    ...
    case "MOVE_STRUCTURE": {
      ...
      if (!s) return { ok: false, message: "structure_not_found" };
      s.x = cmd.x;
      s.y = cmd.y;
      world.revision += 1;
    ...
    case "REMOVE_STRUCTURE": {
      if (!world.structures[cmd.structureId]) return { ok: false, message: "structure_not_found" };
