...
let assetCatalog = { tileSets: [], humans: [], animals: [], items: [], objects: [] };
...
fastify.post("/import-seed", async (req) => {
    const data = await importSeed(req.body.path);
    setWorld({ ...getWorld(), ...data });
    return { ok: true };
});
fastify.post("/spawn/actor", async (req) => {
...
fastify.post("/spawn/item", async (req) => {
    const world = getWorld();
